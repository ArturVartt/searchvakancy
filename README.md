...
- ✅ **Phase 8** — Трекер откликов (`apps/applications`) и «набор для
  отклика». Модель `Application` (один отклик на пару пользователь+вакансия,
  статусы Сохранено → Откликнулся → Собеседование → Оффер/Отказ, заметка,
  использованный сет профиля, `applied_at` проставляется автоматически при
  первом переходе в «отправленный» статус). `POST /api/applications/` —
  upsert по вакансии, чужой сет профиля привязать нельзя. В списке/деталях
  вакансии приходит `application_status`, поэтому карточка сразу показывает
  «✓ Откликнулся» и повторный отклик виден. На фронтенде: кнопка
  «Откликнуться» на карточке открывает окно — выбор сета профиля,
  копирование телефона/почты/GitHub, скачивание резюме, сопроводительное
  письмо из шаблона сета (подстановки `{компания}`/`{должность}`, письмо
  можно править перед копированием), статус и заметка; «Открыть вакансию и
  откликнуться» открывает страницу вакансии и сохраняет отклик. Страница
  `/applications` — список откликов с фильтром по статусам, сменой статуса,
  заметками и удалением. Шаблон письма — новое поле `cover_letter` у сета
  профиля.
...
- `GET/POST /api/applications/` (POST — upsert по `job_id`), `GET/PATCH/DELETE
  /api/applications/<id>/` — свои отклики (см. Phase 8); фильтры `?status=`, `?job=`.
