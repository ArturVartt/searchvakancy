...
⚠️ Важная оговорка, отличающая этот источник от всех остальных в проекте:
`robots.txt` (https://hh.uz/robots.txt) для `User-Agent: *` (то есть для
всех ботов, кроме явно перечисленных) содержит `Disallow: *?*` — блокирует
ЛЮБОЙ URL с query-строкой, а поиск вакансий на этой платформе только через
`?text=&area=` и не имеет path-based альтернативы. Для `User-Agent: Yandex`
у них отдельный, более мягкий блок (`Clean-param` без общего `Disallow`)
— то есть площадка сознательно пускает в свой поисковый индекс конкретно
Яндекс, а не ботов вообще. Формально это тот же случай, что и `trudvsem.ru`/
`Ishkop.uz` (см. README) — оба были отклонены именно по этой причине.

Здесь решение другое: владелец проекта, после объяснения, что `robots.txt`
— не техническое ограничение и не закон (нарушение не влечёт юридических
последствий для небольшого некоммерческого агрегатора вроде этого, риск
чисто практический — бан IP), явно решил подключить источник несмотря на
запрет (22.09.2026, тот же паттерн решения, что и с Zarplata.ru раньше).
Компромисс — не быстрые фиксированные паузы между запросами, как у
остальных скрейперов (1 сек), а случайные "человеческие" паузы
(`HUMAN_DELAY_RANGE`), чтобы не выглядеть explicit-ботом с точки зрения
паттерна трафика — это снижает нагрузку/заметность, но не отменяет сам
факт игнорирования `robots.txt`.
"""
import logging
import random
import time
